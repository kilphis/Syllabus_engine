...
import { timetableData, saveStore, renamePlan } from '../store.js';
...
export function renderTabs() {
    ...
    const allPlans = [...timetableData.planOrder];
    if (timetableData.plans["共有"] || timetableData.currentPlan === "共有") {
        if (!allPlans.includes("共有")) allPlans.push("共有");
    ...
    tabContainer.innerHTML = allPlans.map(plan => {
        ...
        const isShared = plan === "共有";
        return `
            <button onclick="switchTab('${plan}')"
                ondblclick="${isShared ? '' : `window.renameTab('${plan}')`}"
                title="${isShared ? '' : 'ダブルクリックで名前変更'}"
